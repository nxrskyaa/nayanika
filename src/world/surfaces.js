...
export function buildRoad(dirs, terrain, opts = {}) {
  ...
  /**
   * The carriageway is laid as several narrow strips side by side rather than
   * one wide quad, and every strip follows the ground.
   *
   * One quad spanning the full width cannot: the ground under a road rises by
   * as much as 3.8m from kerb to kerb, so a flat span is buried on the uphill
   * side — that is what put wedges of grass through the tarmac. Raising the
   * whole deck onto the high point fixed the tarmac and broke something worse,
   * because the player still walks at ground height and so sank through the
   * road he was standing on. Strips narrow enough to track the terrain mesh
   * keep the surface *at* ground level, which is the only version of this that
   * both looks right and can be walked on.
   */
  const laneWidth = 0.8
  const stripsAcross = (from, to, color, lift, material) => {
    const total = to - from
    const count = Math.max(1, Math.ceil(Math.abs(total) / laneWidth))
    let prev = offsetPath(dirs, from, radius)
    for (let k = 1; k <= count; k++) {
      const off = from + (k / count) * total
      const next = offsetPath(dirs, off, radius)
      const strip = stripBetween(
        toSurfacePoints(prev, terrain, lift),
        toSurfacePoints(next, terrain, lift),
        ups,
        color,
  ...
  stripsAcross(-half, half, roadColor, 0.15, { layer: DECAL_LAYER.road })

  if (pavement > 0) {
    for (const sign of [-1, 1]) {
      stripsAcross(sign * half, sign * (half + pavement), pavementColor, kerbHeight, null)

      const inner = offsetPath(dirs, sign * half, radius)
      ...
      const face = stripBetween(
        ...
        toSurfacePoints(inner, terrain, 0.12),
        sideNormals,
        pavementColor,
      )
      if (face) g.add(face)
  ...
  if (dashed) {
    // Long dash, long gap. Two-station dashes broke into flickering specks as
    // soon as the road ran away from the camera.
    const seg = 4
    for (let i = 0; i + seg <= dirs.length - 1; i += seg * 2) {
      ...
      const dash = stripBetween(
        toSurfacePoints(offsetPath(slice, -0.22, radius), terrain, 0.2),
        toSurfacePoints(offsetPath(slice, 0.22, radius), terrain, 0.2),
        sUps,
        lineColor,
  ...
  if (edgeLines) {
    for (const sign of [-1, 1]) {
      const line = stripBetween(
        toSurfacePoints(offsetPath(dirs, sign * (half - 0.6), radius), terrain, 0.2),
        toSurfacePoints(offsetPath(dirs, sign * (half - 0.34), radius), terrain, 0.2),
        ups,
        lineColor,
        { material: { layer: DECAL_LAYER.marking } },
      )
